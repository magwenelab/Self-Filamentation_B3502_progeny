...
## Write a ftn to draw chromosomes
def drawchroms(chrlens,ymod = 0.2,divide_by = 10**6,linecolor='grey',fontsize=12,linewidth= 0.5,
               chromnames=None,ax=None,figsize=(10,10),facecolor='w',xleft=0,tickleft=2,
               tickright=-1,xlabel=None,ylabel=None):
    """
    Draws a figure of chromosome models given the lengths in CHRLENS.
    """
    ## Rename chromnames if they are not needed
    if chromnames is None:
        chromnames = np.arange(len(chrlens))+1
    ...
    ## Iterate thru each chromosome length
    for i,j in enumerate(chrlens):
        
        ## Plot horizontal ines
        plt.hlines((i-ymod,i+ymod),xleft,j,color=linecolor,linewidth=linewidth)
        
        ## Plot vertica lines
        plt.vlines((xleft,j),i-ymod,i+ymod,color=linecolor,linewidth=linewidth)
     
    ## Adjust x- and yticks
    plt.yticks(np.arange(len(chromnames)),chromnames,fontsize=fontsize);
    p,l = plt.xticks()
    plt.xticks(p[tickleft:tickright],p[tickleft:tickright]/divide_by,fontsize=fontsize)
    
    ## Add x- and ylabel
    plt.xlabel('Chromosome Coordinate (Mb)' if xlabel is None else xlabel,fontsize=fontsize)
    plt.ylabel('Chromosome' if ylabel is None else ylabel,fontsize=fontsize)
...
def findchrom_v0(df,c, chrom='Chrom', pos='Pos', bounds = None):
    """
...
## subset a chromosome variants
def findchrom(df,c, chrom='Chrom'):
    """
    Splits a dataframe (DF) on the column CHROM where the 
    value of CHROM is C. DF must be a dataframe were rows 
    are genetic variant sites with column named CHROM
    representing the chormosome.

    """
    return df[(df[chrom]==c)]

## write a ftn that applys a median filter to a pandas 
## data frame of genetic values
def median_chrom(genotypes,mappop,centdf,c,kernel=11):
    temp = findchrom(genotypes,c).copy()
    cent = findchrom(centdf,c)[['Start','End']].values
    
    left = temp[(temp.Pos<=cent.max())][::-1]
    right = temp[(temp.Pos>=cent.min())]
    
    mleft =  left[mappop].rolling(kernel).median()
    mrigh = right[mappop].rolling(kernel).median()
    
    temp.loc[mleft.index,mappop] = mleft.values
    temp.loc[mrigh.index,mappop] = mrigh.values
...
def haplotypemap(geno,mappop,
    ...
    if (c is None):
        ...
        pass
    ctemp = findchrom(geno,c,chrom=chrom)
    ...
    if centromerepath is not None:
        ...
        x = np.arange(*findchrom(centromeres,c)[['Start','End']].values[0])
        y = np.ones(len(x))
    ...
    if filter and (centromerepath is not None):
        ctemp_m = median_chrom(geno,mappop,centromeres,c,kernel=kernel_size)
        co_s = ctemp_m[mappop].dropna(axis=0,how='all'
                                ).diff(axis=0).dropna(axis=0,how='all'
                                ).abs().sum(axis=1)

        cof = co_s.values/len(mappop)
        plt.plot(ctemp_m.loc[co_s.index,pos], cof,
            color='k',alpha=0.5)
...
def manhattan(df,
    ...
    for i,(ci,c) in enumerate(chrmap.groupby(chrom)):
        ...
        if vlines:
            ...
            plt.vlines(temp[pos].values+cumpos,np.zeros(len(temp)),temp[pval].values,
                 linewidth=linewidth,rasterized=rast,color=cmap[int(i%len(cmap))])

        
    if (threshold is not None):
        plt.hlines(threshold,0,chrmap.Cumlen.max()+chrmap.Length.tolist()[-1],
                   color='k',linestyle='--',alpha=0.2,linewidth=0.5)
...
## Plot a histogram and quantile plot of the phenotype
def histqq(pheno_df,
    parent_labels = [],
    parent_colors = ['tab:blue','tab:orange'],
    parent_line_len = None,
    parent_alpha = 0.8,
    parent_marks = ['^','o'],
    parent_size = 5,
    marker_size = 5,
    ...
    histcolor='tab:grey',
    histalpha=0.5,
    historientation='horizontal',
    histylabel = 'Phenotype',
    histxlabel = 'Number of F$_1$ segregants',
    hist_leg_loc = 0,
    qcolor = 'tab:grey',
    qalpha = 0.5,
    qmarker='.',
    ...
    """
    Plots a paired histogram (left) and Q-Q plot (right) of a phenotype.
    The phenotype is expected to be a dataframe with an index of sample names.
    Will plot and mark parents if parental names are listed in parent_labels (default = []).
    """

    if (ax is None):
        fig,ax = plt.subplots(rown,coln,figsize=figsize,sharey=sharey)
    else:
        pass

    ishoriz = historientation is 'horizontal'
    phenoname = pheno_df.columns.min()
    ...
    ah = ax.ravel()[col1]
    plt.sca(ah)
    plt.hist(pheno_df[phenoname].values,
         color=histcolor,alpha=histalpha,orientation=historientation)
    ...
    ## Set y and x lables
    plt.ylabel(histylabel if ishoriz else histxlabel,fontsize=fontsize)
    plt.xlabel(histxlabel if ishoriz else histxlabel,fontsize=fontsize)

    p,l = plt.xticks() if ishoriz else plt.yticks()

    if verbose:
        print(p)

    if parent_line_len is None:
        parent_line_len = p[1]/line_ratio

    ## Plot parents
    for i,parent in enumerate(parent_labels):
        parent_pheno = pheno_df.loc[parent].min()
        if ishoriz:
            plt.hlines(parent_pheno,0,parent_line_len,
                alpha=parent_alpha,label=parent,color=parent_colors[i])
        else:
            plt.vlines(parent_pheno,0,parent_line_len,
                alpha=parent_alpha,label=parent,color=parent_colors[i])
    ...
    ## Add legend
    if legend:
        plt.legend(fontsize=fontsize,loc=hist_leg_loc)
    ...
    ## Sort phenotype
    spheno = pheno_df.sort_values(phenoname).copy()  
    samples = [s for s in spheno.index.tolist() if s not in parent_labels]
    x = np.arange(len(samples))

    plt.plot([np.min(x),np.max(x)],[spheno[phenoname].min(),spheno[phenoname].max()],
             linestyle=qlinestyle,color=qlinecolor,alpha=qlinealpha,linewidth=qlinewidth)

    plt.plot(x,spheno.loc[samples,phenoname].values,
                qmarker,color=qcolor,alpha = qalpha, ms = marker_size)

    for i,parent in enumerate(parent_labels):
        px = spheno.index.tolist().index(parent)
        plt.plot(px,pheno_df.loc[parent,phenoname].min(),parent_marks[i],
                    color=parent_colors[i],alpha=parent_alpha,
                    label=parent,ms=parent_size);

    plt.xlabel(qxlabel,fontsize=fontsize)

    ## add legend
    if legend:
        plt.legend(fontsize=fontsize,loc=q_leg_loc)

    ## adjust space between plots
    plt.subplots_adjust(wspace=spacewide,hspace=spacehigh)
...
## Returns the max indexes along the chromosome positon and funcitonal index for QTL 
