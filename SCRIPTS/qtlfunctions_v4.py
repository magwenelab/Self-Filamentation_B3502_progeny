...
def loadgff(path,sep='\t',header=None,comment='#',
            genecol = 'Gene',strandcol = 'Strand',parentcol='Parent',
            genesplit = '-t26',parentsplit = 'Parent=',idsplit = 'ID=',
...
def chrommap(df,chrom='Chrom',pos='Pos',
...
def findchrom(df,c,chrom='Chrom',pos='Pos',bounds = None):
    ...
    return df[(df[chrom]==c) & (df[pos]>=np.min(bounds)) & (df[pos]<=np.max(bounds))].sort_values(pos)

## Generate a haplotype map; e.g. figure 1 from Roth et al. 2018
def haplotypemap(geno,mappop,c=None,plotseg=None,ns=10,
           ws=10000,slide=2500,savepath=None,
           chrom='Chrom',pos='Pos',parent_labels = None,
           bicolor=['tab:blue','tab:orange'],
           marker ='.',hspace = 0.4,fs = 12,
           figsize=(10,5),cobuff = 0.05,mydpi=200,
           samplelabelsize=6,y_jitter=0.05,
           legendbox = (-0.07,0.5),
           legendscale=6,legendtitle='Genotype'):
    """
    ...
    for i,s in enumerate(plotseg):
        stemp = ctemp[[pos,s]]
        for j,g in enumerate(sorted(stemp[s].unique())):
            hap = stemp[(stemp[s]==g)]
            plt.plot(hap[pos],
...
def calcci(df,ftn,mappop,pheno,
           nboots=500,alpha=5,bootnames = ['l','r','QTL']):
    """
    ...
    while len(boots)<nboots:
        ...
        l,r,m,ix = findpeak(qtl)
        
        boots.append((l,r,m))
    
    boots = pd.DataFrame(boots,columns=bootnames)
    cil = np.percentile(boots.l.values,alpha/2)
    cir = np.percentile(boots.r.values,100-(alpha/2))
    return cil,cir,boots
    
## Establish a null distribution using permutation tests
def QTLperm(df,ftn,mappop,pheno,n=1000,alpha=5):
    """
    ...
    perms = []
    while len(perms) < n:
        perms.append(QTLmap(df,ftn,np.random.permutation(mappop),pheno)['QTL'].max())
    return np.percentile(perms,100 - alpha),perms
        
## Display a manhattan plot
def manhattan(df,pval='QTL',pos='Pos',chrom='Chrom',
              chrmap = None, ax = None, threshold = None,
              figsize=(12,3), savepath = None,
              ms =1, fs =12, rast = True, mydpi = 200, 
              ylabel = pvalylable,
              xlabel = 'Chromosomes'):
    """
    ...
    for ci,c in chrmap.groupby(chrom):
        temp = df[(df[chrom]==ci)]
        cumpos = c.Cumlen.min()
    ...
    plt.xticks(chrmap.Midpts.values,chrmap[chrom].values,fontsize=fs)
...
def qtldf(df,pheno,mappop,qtl='QTL',
...
def longitudinal_plot(logp,ftnix,ax=None,chrmap=None,
                      chrom='Chrom',pos='Pos',
